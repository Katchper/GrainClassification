...
class PythonSandbox:
    ...
    def __init__(self):
        ...
        self.ML_model = cv2.ml.RTrees_load("random_forest_model.xml")
    ...
    def process(self, inframe, outframe):
        ...
        for i, cnt in enumerate(contours):
            ...
            if self.min_area < area_temp:
                ...
                finalList = [total_hue, total_sat, total_val, hm[0], hm[1], hm[2], hm[3], hm[4], hm[5], hm[6], hm[7], circularity, circleRatio, rectangularity, aspect_ratio, compactness]
                _, predictions = self.ML_model.predict(np.array(finalList, dtype=np.float32))
                if predictions[0][0] == 2.0:
                    cv2.putText(cropped_image2, "whole", (x1, y1),cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 3)
                elif predictions[0][0] == 1.0:
                    cv2.putText(cropped_image2, "groat", (x1, y1),cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 3)
                else:
                    cv2.putText(cropped_image2, "broken", (x1, y1),cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 3)

                cv2.circle(cropped_image2, (x1 + round(w / 2), y1 + round(h / 2)), 1, (0, 255, 255), 5)
                cv2.drawContours(cropped_image2, [cnt], -1, (0, 0, 255), 2)

        # Write a title:
        # Write frames/s info from our timer into the edge map (NOTE: does not account for output conversion time):
        fps = self.timer.stop()
        cv2.putText(cropped_image2, str(fps), (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 3)
